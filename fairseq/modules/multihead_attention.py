...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        # 添加RoPE支持
        self.use_rope = use_rope and self_attention  # 只在self-attention中使用RoPE
        if self.use_rope:
    ...
    def forward(
        ...
        if self.self_attention:
            ...
            assert key is not None and value is not None
            q = self.q_proj(query) #shape: tgt_len: 13, bsz:288 , embed_dim:512                                                        
            k = self.k_proj(key)  #shape: src_len: 14, bsz:288 , embed_dim:512
            v = self.v_proj(value)
        q *= self.scaling
        ...
        # 在q,k投影和scaling之后，reshape之前应用RoPE
        if self.use_rope:
            # 1. 确定序列长度和位置偏移
            if incremental_state is not None:
                # 推理时：每次处理一个token
                ...
                position_offset = 0
                if saved_state is not None and "prev_key" in saved_state:
                    # prev_key shape: [bsz, num_heads, seq_len, head_dim]
                    prev_key = saved_state["prev_key"]
                    # 获取累积的序列长度
                    position_offset = prev_key.size(2)  # 使用实际的序列长度
                    # print(position_offset)
                
                # 确保生成足够长的位置编码
                seq_len = position_offset + 1  # 当前位置的编码
            else:
                # 训练时：处理完整序列
                seq_len = tgt_len
                position_offset = 0
            ...
            # 获取旋转位置编码
            cos, sin = self.rotary_emb(query, seq_len)  # [seq_len, 1, 1, dim]

            # 重塑为4D以适应RoPE
            q_4d = q.view(tgt_len, bsz, self.num_heads, self.head_dim)
            if k is not None:
                k_4d = k.view(src_len, bsz, self.num_heads, self.head_dim)
            
            # 应用RoPE
            q_4d, k_4d = apply_rotary_pos_emb(q_4d, k_4d, cos, sin, offset=position_offset)
            
            # 恢复为3D
            q = q_4d.reshape(tgt_len, bsz, self.embed_dim)
            if k is not None:
                k = k_4d.reshape(src_len, bsz, self.embed_dim)

        q = (
            q.contiguous()
            .view(tgt_len, bsz * self.num_heads, self.head_dim) #shape: bsz*num_heads:288*8 ,tgt_len: 13,  head_dim:64
            .transpose(0, 1)
        )
        ...
        if k is not None:
            ...
            k = (
                k.contiguous()
                .view(-1, kv_bsz * self.num_heads, self.head_dim) #shape: bsz*num_heads:288*8 ,src_len: 14,  head_dim:64
                .transpose(0, 1)
            )
        if v is not None:
            v = (
    ...
    def reorder_incremental_state(
        ...
        if input_buffer is not None:
            for k in list(input_buffer.keys()):  # 使用list复制键，因为我们可能会修改字典
